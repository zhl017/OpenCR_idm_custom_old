...
void publishSensorStateMsg(void)
{
  ...
  dxl_comm_result = motor_driver.readEncoder(sensor_state_msg.left_encoder, sensor_state_msg.right_encoder, sensor_state_msg.rear_left_encoder, sensor_state_msg.rear_right_encoder);

  if (dxl_comm_result == true)
    updateMotorInfo(sensor_state_msg.left_encoder, sensor_state_msg.right_encoder, sensor_state_msg.rear_left_encoder, sensor_state_msg.rear_right_encoder);
  else
    return;
...
*******************************************************************************/
void updateMotorInfo(int32_t left_tick, int32_t right_tick, int32_t rear_left_tick, int32_t rear_right_tick)
{
  ...
  if (init_encoder)
  {
    ...
    last_tick[REAR_LEFT] = rear_left_tick;
    last_tick[REAR_RIGHT] = rear_right_tick;
...
{
  float* orientation;
  double wheel_l, wheel_r, wheel_rl, wheel_rr;      // rotation value of wheel [rad]
  ...
  wheel_rl = TICK2RAD * (double)last_diff_tick[REAR_LEFT];
  wheel_rr = TICK2RAD * (double)last_diff_tick[REAR_RIGHT];
  ...
  delta_x = WHEEL_RADIUS * (wheel_r + wheel_l + wheel_rr + wheel_rl) / 4.0;
  delta_y = WHEEL_RADIUS * (wheel_r - wheel_l - wheel_rr + wheel_rl) / 4.0;
  ...
  last_velocity[REAR_LEFT]  = wheel_rl / step_time;
  last_velocity[REAR_RIGHT] = wheel_rr / step_time;
  last_theta = theta;
...
void driveTest(uint8_t buttons)
{
  ...
  motor_driver.readEncoder(current_tick[LEFT], current_tick[RIGHT], current_tick[REAR_LEFT], current_tick[REAR_RIGHT]);
  ...
  if (buttons & (1<<0))  
  {
    ...
    diff_encoder = TEST_DISTANCE / (2 * 3.14159265359 * WHEEL_RADIUS / 4096); // (Circumference of Wheel) / (The number of tick per revolution)
    tTime[6] = millis();
  }
  else if (buttons & (1<<1))
  {
    ...
    diff_encoder = (TEST_RADIAN * TURNING_RADIUS) / (2 * 3.14159265359 * WHEEL_RADIUS / 4096);
    tTime[6] = millis();
  }
  ...
  {    
    ...
    {
      goal_velocity_from_button[LINEAR_X]  = 0.20;  // max vel : 0.40
      tTime[6] = millis();
    }
    else
    {
  ...
  {   
    if (abs(saved_tick[RIGHT] - current_tick[RIGHT]) <= diff_encoder)
    {
      goal_velocity_from_button[ANGULAR]= -1.0; // max ang : 1.6
      tTime[6] = millis();
    }
    else
    {
...
{
  static char *joint_states_name[] = {(char*)"wheel_left_joint", (char*)"wheel_right_joint", (char*)"wheel_rear_left_joint", (char*)"wheel_rear_right_joint"};
...
void updateGoalVelocity(void)
{
  ...
  sensors.setLedPattern(goal_velocity[LINEAR_X], goal_velocity[ANGULAR]);
}
...
void sendDebuglog(void)
{
  ...
  motor_driver.readEncoder(encoder[LEFT], encoder[RIGHT], encoder[REAR_LEFT], encoder[REAR_RIGHT]);
  
  DEBUG_SERIAL.println("Encoder(left) : " + String(encoder[LEFT]));
  DEBUG_SERIAL.println("Encoder(right) : " + String(encoder[RIGHT]));
  DEBUG_SERIAL.println("Encoder(rear_left) : " + String(encoder[REAR_LEFT]));
  DEBUG_SERIAL.println("Encoder(rear_right) : " + String(encoder[REAR_RIGHT]));
